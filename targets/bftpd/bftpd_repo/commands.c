...
void command_pass(char *password)
{
	...
	if (bftpd_login(password)) {
		...
		/*
                This is printed for us in the login function.
...
void command_type(char *params)
{
    if ((*params == 'A') || (*params == 'a')) {
        ...
        control_printf(SL_FAILURE, "500 Type needs a parameter.");
}
...
void command_pasv(char *foo)
{
    ...
    if (epsvall) {
        ...
        /* avoid overflow by closing any previously existing passive sockets */
        if (pasvsock >= 0)
          close(pasvsock);
...
void command_epsv(char *params)
{
    ...
    /* avoid overflow from multiple passive sockets */
    if (pasvsock >= 0)
       close(pasvsock);
    pasvsock = socket(AF_INET, SOCK_STREAM, 0);
    sa.sin_addr.s_addr = INADDR_ANY;
...
void command_rnto(char *newname)
{
	char *mapped = bftpd_cwd_mappath(newname);
        if ( (! mapped) || (! philename) )
        {
           if (mapped) free(mapped);   // avoid leaking memory
